...
/**
 * Where a case study can go from where it is. Editorial workflow rather than a
 * free status dropdown: an archived study is restored as a draft so it gets a
 * second read — and a client's sign-off re-checked — before it is public again.
 */
...
export default function CaseStudyRowActions({
  caseStudyId,
  status,
  title,
  slug,
  ...
  async function run(label: string, request: () => Promise<Response>) {
    ...
    if (!res.ok) {
      ...
      alert(body?.error?.message ?? `Could not ${label.toLowerCase()} this case study`);
    }
    setBusy(null);
    router.refresh();
  }
  ...
  const changeStatus = (label: string, next: string) =>
    run(label, () =>
      fetch(`/api/proxy/admin/case-studies/${caseStudyId}/status`, {
        method: 'PATCH',
  ...
  const duplicate = () =>
    run('Duplicate', () => fetch(`/api/proxy/admin/case-studies/${caseStudyId}/duplicate`, { method: 'POST' }));

  const remove = () => {
    if (!confirm(`Delete “${title}”? This cannot be undone.`)) return;
    return run('Delete', () => fetch(`/api/proxy/admin/case-studies/${caseStudyId}`, { method: 'DELETE' }));
  };

  return (
    <div ref={menuRef} className="relative flex justify-end gap-1.5">
      <a href={`/case-studies/${caseStudyId}/edit`} className="btn-subtle px-2 py-1 text-[13px]">
        Edit
      </a>
      {status === 'published' && (
        <a
          ...
          title="Open the live case study"
        >
          View ↗
        </a>
      )}
