 ...
 * the results stay server-rendered. Submitting works without JavaScript, and
 * pressing Enter in the search box submits it like any other form.
 */
export default function JobsToolbar({
  ...
  const hasFilters = Boolean(search || department || employmentType || status);
  ...
  return (
    <form method="get" className="flex flex-wrap items-center gap-2 border-b border-ink-200 px-5 py-3.5">
      ...
      <select name="department" className="field w-auto" defaultValue={department ?? ''} aria-label="Filter by department">
      ...
      <select name="employmentType" className="field w-auto" defaultValue={employmentType ?? ''} aria-label="Filter by type">
      ...
      {hasFilters && (
        <Link href="/jobs" className="btn-subtle px-2.5 py-1.5 text-[13px]">
          Clear
        </Link>
      )}
    </form>
